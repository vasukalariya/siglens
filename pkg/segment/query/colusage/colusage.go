...
func GetFilterAndQueryColumns(astNode *structs.ASTNode, aggs *structs.QueryAggregators) (map[string]struct{}, map[string]struct{}) {
	// For generating commands, no columns in the query are ingested.
	if isGeneratingCommand(aggs) {
	...
	queryCols := GetQueryColumns(aggs)
	utils.MergeMapsRetainingFirst(queryCols, filterCols)

	return filterCols, queryCols
}

func GetQueryColumns(aggs *structs.QueryAggregators) map[string]struct{} {
	if isGeneratingCommand(aggs) {
...
func isGeneratingCommand(aggs *structs.QueryAggregators) bool {
	if aggs != nil {
